...
final class CustomExercise {
    ...
    /// Optional user photo. Kept out of the main store file so a big image never bloats it.
    ...
    init(id: String = "custom-\(UUID().uuidString)", name: String,
         ...
         category: ExerciseCategory, instructions: [String] = [], imageData: Data? = nil) {
        self.id = id
        self.name = name
    ...
    var asExercise: Exercise {
        Exercise(
            id: id,
            name: name,
            force: forceRaw.flatMap(Force.init(rawValue:)),
            level: Level(rawValue: levelRaw) ?? .beginner,
            mechanic: mechanicRaw.flatMap(Mechanic.init(rawValue:)),
            equipment: equipmentRaw.flatMap(Equipment.init(rawValue:)),
            primaryMuscles: primaryMusclesRaw.compactMap(Muscle.init(rawValue:)),
            secondaryMuscles: secondaryMusclesRaw.compactMap(Muscle.init(rawValue:)),
